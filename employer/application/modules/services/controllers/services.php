...
class Services extends MX_Controller{
    public function __Construct()
    {
        ...
        $this->load->model('services_model','content');
...
//                    exit;
    ...
    public function slider_content($content_id){
        if(strlen($content_id) <= 0){ redirect('services'); }
        ...
        if($this->input->post()){
            ...
            if(strlen($_FILES['image']['name']) > 0){
                ...
                if(in_array($extension,$allowed_type)){
                    ...
                    redirect('services/list_photos/'.$this->input->post('content_id'));
                }
            }else{
                if($id > 0){
...
//code to add new tags in content
    ...
    public function send($content_id)
    {
        ...
        if(! empty($content_detail) && $content_detail['content_type'] == 'Article' && ! empty($subscribes))
        {
            ...
            $server_url = $site_settings['site_url'].'/index.php/services/detail/'.$content_detail['content_url'];
