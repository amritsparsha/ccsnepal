...
class Career_resources extends CI_Controller{
    private $per_page;
    public function __Construct()
    {
        parent::__Construct();
        $this->load->model('site_settings_model', 'settings');
        $this->load->model('career_resources_model','career');
    ...
    {
            $detail= $this->career->get_detail_resources($ref_no);
            $data['content']= $detail;
            ...
            $this->load->view('career_resources/career_resources_detail');
            $this->load->view('footer');
    }
     public function all_resources($start_point='')
    {
            $all_resources=$this->career->get_all_resources();
            ...
            $this->load->view('header', $data);
            $this->load->view('career_resources/career_resources_list');
            $this->load->view('footer');
    }
    //paginations
    ...
    public function pagination($id,$start_point,$count,$page)
    {
            if($id=="0")
            {
                $config['base_url'] =  site_url('career_resources/'.$page);
                $config['uri_segment'] = 3;
            }
            else
            {
                $config['base_url'] =  site_url('career_resources/'.$page.'/'.$id);
