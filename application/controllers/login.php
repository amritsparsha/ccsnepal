...
class Login extends CI_Controller{
        ...
        if($this->input->post())
        {
            ...
            if ($this->form_validation->run()) {
                ...
                $password = md5($this->input->post('password'));
                    ...
                    //to upload cv redirected from upload cv
                        if(!empty($this->session->userdata('upload_cv_flag')) && $this->session->userdata('upload_cv_flag') == '1')
                        {
                            $this->insert_updated_cv();
                        }
                    //
                        redirect('home');
                    } 
    ...
    public function insert_updated_cv()
    {
                    ...
                    $previous_cv = $this->crud->get_where("tbl_cust_uploaded_cv",array('publish_status'=>1,'delete_status'=>0,'customer_id'=>$customer_id));
                    foreach ($previous_cv as $key => $cv) 
                    {
                        $update['publish_status']='0';
                        $update['delete_status']='1';
                        $this->crud->update($cv['cuc_id'],'cuc_id',$update,"tbl_cust_uploaded_cv");
                    }
                    //end
                    
                    $result=$this->crud->update($this->session->userdata('cuc_id'),'cuc_id',$insert,"tbl_cust_uploaded_cv");

                    if($result)
                    {
                        $this->session->set_userdata('success',"Successfully Uploaded Cv."); 
                        $this->session->unset_userdata('uc_email');
                        $this->session->unset_userdata('cuc_id');
                        $this->session->unset_userdata('upload_cv_flag');
                        redirect('home');
                    }
                    else
                    {
...
//            exit;
                ...
                if($this->send_account_activation_mail( $insert['email'],  $insert['activation_code']) == "TRUE")
                {
                    ...
                    if ($result) 
                    {
                        if($cuc_id != null)
                        {  
                        //to keep id of customer in uploaded cv databse
                            $update['customer_id']=$result;
                            $this->crud->update($cuc_id,'cuc_id', $update, 'tbl_cust_uploaded_cv');
                            $this->session->unset_userdata('uc_email');
                            $this->session->unset_userdata('cuc_id');
                            $this->session->unset_userdata('upload_cv_flag');
                        //end
...
<div style="width:580px;margin:0 auto;font:12px Arial,Helvetica,sans-serif;background:#fff">
  <div style="margin:0 0 10px"> <img alt="Rupakto Tours and Travels" src="'.base_url().'/themes/images/logos/navbar-logo.png"> </div>
    ...
    <table width="100%" cellspacing="0" cellpadding="5" border="0">
        ...
        <tr>
            <td colspan="2" style="background:#800000; text-align:left;">Thanks and Regards,<br />
                Rupakto Tours and Travels Team<br />
                 Tel:<strong>+977-01-4437892</strong><br />
                <a href="https://www.rupakottravels.com/" target="_blank" style="color:#46216F; text-decoration:underline;">www.rupakottravels.com</a></td>
        </tr>
    ...
    public function account_activation($code)
    {
        ...
        if(!empty($detail))
        {
            ...
            // to get detail of site_users
            $site_users=$this->crud->get_detail($code,'activation_code','igc_site_users');
        // to upload cv for activated site users
            $update_cv['delete_status'] = "0";
            $update_cv['publish_status'] = "1";
            $update_cv['updated'] = date('Y-m-d:H:i:s');
            $this->crud->update($site_users['customer_id'],'customer_id', $update_cv, 'tbl_cust_uploaded_cv');
        // end of code of uploading cv
