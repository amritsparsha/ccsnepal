...
class jobs extends MX_Controller{
    ...
    public function index()
    {
        ...
        $data['jobs'] = $this->crud->get_where_order($table,array('admin_ref >'=>0,'delete_status'=>0),"created","DESC");
    ...
    {
        $table = 'tbl_job_post_all';
        $data['row'] = $this->crud->get_detail($id,"job_id",$table);
        $data['title']= "Jobs Detail ";
        $this->load->view('header', $data);
        $this->load->view('jobs_ind_view');
        $this->load->view('footer');
    }
    public function expired()
    {
        ...
        $data['jobs'] = $this->crud->get_where_order($table,array('admin_ref >'=>0,'delete_status'=>0,'deadline <'=>$date),"created","DESC");
        $data['title']= "Expired Jobs List";
        $this->load->view('header', $data);
        $this->load->view('jobs_list');
        $this->load->view('footer');
    }
    public function pending()
    {
        $table = 'tbl_job_post_all';
        $date = date('Y-m-d:H:i:s');
        $data['jobs'] = $this->crud->get_where_order($table,array('admin_ref >'=>0,'delete_status'=>0,'publish_status'=>'1'),"created","DESC");
        $data['title']= "Pending Jobs List";
        $this->load->view('header', $data);
        $this->load->view('jobs_list');
        $this->load->view('footer');
    }
    public function draft()
    {
        $table = 'tbl_job_post_all';
        $date = date('Y-m-d:H:i:s');
        $data['jobs'] = $this->crud->get_where_order($table,array('admin_ref >'=>0,'delete_status'=>0,'publish_status'=>'0'),"created","DESC");
        $data['title']= "Draft Jobs List";
        $this->load->view('header', $data);
        $this->load->view('jobs_list');
        $this->load->view('footer');
    }
    ...
    public function form($id=0)
    {
        if($this->input->post())
        {
            ...
            $insert['employer_ref'] =$this->input->post('admin_employer_ref'); 
        ...
        // 7-publish
            $insert['super_publish_status'] = $this->input->post('publish_status');
            $insert['publish_status'] = $this->input->post('publish_status');
        //
            $folder_path = '../uploads/jobs/';
            $rand = md5(rand());
        ...
        $data['employers']=$this->crud->get_where("tbl_company_employers",array("delete_status"=>"0","publish_status"=>"1","active_status"=>"Y","company_type"=>"2"));
